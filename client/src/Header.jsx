...
function Header({ handleSideToggle }) {
  ...
  return (
    <header className='text-inherit w-full px-8 sm:px-16 py-3 flex justify-between items-center gap-4 bg-inherit border-b border-inherit z-10'>
      ...
      <Form className={`${isSearchOpen ? 'flex' : 'hidden'} w-full h-full md:w-1/2 px-3 py-1 md:flex justify-end items-center border gap-2`} role='search'>
        <button
          type='reset'
          onClick={() => {
            handleSearchToggle();
        ...
        <input name='q' id='q' type="search" defaultValue={q} autoComplete='off' autoCorrect='off' className='w-full font-semibold p-0 bg-inherit border-0 focus:ring-0 active:ring-0' />
